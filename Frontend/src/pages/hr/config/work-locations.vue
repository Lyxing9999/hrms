        ...
        >
          <template #default="{ row }">
            <div class="table-actions">
              ...
              <el-tooltip
                :content="row.lifecycle.deleted_at ? 'Restore Location' : 'Delete Location'"
                placement="top"
              >
                <el-button
                  ...
                  plain
                  @click="row.lifecycle.deleted_at ? restoreLocation(row) : deleteLocation(row)"
          ...
          <el-col :xs="24" :lg="12">
            ...
            <el-form-item label="Radius (meters)" prop="radius_meters">
              ...
              <div class="form-help-text">
                Attendance tracking radius: {{ formatRadius(formData.radius_meters || 0) }}
              </div>
            </el-form-item>
...
const formRules: FormRules = {
  ...
  address: [
    ...
    { min: 5, max: 500, message: 'Address must be 5-500 characters', trigger: 'blur' },
  ],
  latitude: [
    { required: true, message: 'Latitude is required', trigger: 'blur' },
    { type: 'number', min: -90, max: 90, message: 'Invalid latitude', trigger: 'blur' },
  ],
  longitude: [
    { required: true, message: 'Longitude is required', trigger: 'blur' },
    { type: 'number', min: -180, max: 180, message: 'Invalid longitude', trigger: 'blur' },
  ],
  radius_meters: [
    { required: true, message: 'Radius is required', trigger: 'blur' },
    { type: 'number', min: 1, max: 10000, message: 'Radius must be 1-10000 meters', trigger: 'blur' },
  ],
};
...
async function loadData() {
  ...
  try {
    ...
    if (error.message?.includes('network') || error.message?.includes('fetch')) {
      ...
      errorMessage = 'Access denied. You may not have permission to view work locations.';
    }
